...
$invoiceNumber = mysqli_query($conn, "select invoice_pembelian_number_id, invoice_pembelian_number_input, invoice_pembelian_number_delete from invoice_pembelian_number where invoice_pembelian_number_parent = " . $di . " && invoice_pembelian_number_user = " . $userLogin . " && invoice_pembelian_cabang = " . $sessionCabang . " ORDER BY invoice_pembelian_number_id DESC LIMIT 1");
$inParent = mysqli_fetch_assoc($invoiceNumber);
        ...
        <div class="card-invoice">
          <div class="d-flex align-items-center">
            ...
            <form role="form" id="form-tambah-invoice" method="post" action="transaksi-pembelian-input-no-invoice.php">
              <input name="invoice_pembelian_number_input" type="text" readonly style="border: 1px solid #eaeaea;" title="Kode Unik Invoice, diambil dari 8 karakter pertama dari hasil md5 microtime">
              <input type="hidden" name="invoice_pembelian_number_parent" value="<?= $di; ?>">
              <input type="hidden" name="invoice_pembelian_number_user" value="<?= $userLogin; ?>">
              <input type="hidden" name="invoice_pembelian_cabang" value="<?= $sessionCabang; ?>"></script>
            </form>
          </div>
          <script>
            if (localStorage.getItem('noInvoice') === null) {
              localStorage.setItem('noInvoice', 'INV-<?php echo substr(md5(microtime()), 0, 8); ?>');
              document.querySelector('input[name="invoice_pembelian_number_input"]').value = localStorage.getItem('noInvoice');
              submitFormTambahInvoice();

              function submitFormTambahInvoice() {
                  var dataform = $('#form-tambah-invoice').serialize();
                  $.ajax({
        ...
        <tbody>
          ...
          ?>
            <tr>
              ...
              <td>
                <span id="harga_keranjang_<?php $bik ?>">Rp. <?= number_format($row['keranjang_harga'], 0, ',', '.'); ?></span>
              ...
              <td style="text-align: center; width: 11%;">
                <form role="form" id="form-edit-qty-<?= $bik ?>" action="FetchRequest/transaksi-pembelian-update-qty.php" method="post">
                  <input type="hidden" name="keranjang_id" value="<?= $row['keranjang_id']; ?>">
                  <input type="hidden" name="barang_id" value="<?= $row['barang_id']; ?>">
                  <input type="number" min="1" name="keranjang_qty" value="<?= $row['keranjang_qty'] ?>" onchange="editKeranjangQty('form-edit-qty-<?= $bik ?>')" onkeypress="return hanyaAngka(event)" style="text-align: center; width: 60%;">
                ...
                <script>
                  function editKeranjangQty(form) {
                    var dataform = $(`#${form}`).serialize();

                    $.ajax({
                      url: "FetchRequest/transaksi-pembelian-update-qty.php",
                      data: dataform,
                      type: "post",
                      success: function(result) {
                        var hasil = JSON.parse(result);
              ...
              <td id="sub_total_keranjang_<?php $bik ?>">Rp. <?= number_format($sub_total, 0, ',', '.'); ?></td>
                ...
                <tr>
                  ...
                  <td>
                    ...
                    <div class="payment">
                      ...
                      $keranjang = mysqli_query($conn, "select keranjang_harga from keranjang_pembelian where keranjang_harga > 0 && keranjang_id_kasir = " . $idKasir . " && keranjang_cabang = " . $sessionCabang . " ");
                      ...
                      <?php }else{ ?>
                        <button class="btn btn-primary" type="">Simpan Payment <i class="fa fa-shopping-cart"></i></button>
                      <?php } ?>
...
<script>
  $('[name="updateStock"]').click(function(){
    localStorage.clear();
  });
