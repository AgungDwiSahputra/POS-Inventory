...
if( isset($_POST["inputbarcodeNonFisik"]) ){
  ...
  if( tambahKeranjangBarcodeNonFisik($_POST) > 0 ) {
    echo "
      <script>
...
if ( isset($_POST["updateHargaJasa"]) ) {
    if( updateQTYHargaNonFisik($_POST) > 0 ) {
      echo "
        <script>
          document.location.href = '';
        </script>
...
if( isset($_POST["updateStock"]) ){
  ...
  if( $hasilquery == 0){
      if( updateStockNonFisik($_POST) > 0 ) {
        echo "
          <script>
...
if( isset($_POST["updateStockDraft"]) ){
  ...
  if( $hasilquery == 0){
      // cek apakah data berhasil di tambahkan atau tidak
      if( updateStockNonFisikDraft($_POST) > 0 ) {
        echo "
          <script>
            document.location.href = '';
...
<!-- Content Wrapper. Contains page content -->
  <div class="content-wrapper">
        ...
        <div class="row mb-2">
          <div class="col-sm-8">
            ...
            <div class="btn-cash-piutang">
              ...
              <?php if ( $r == 1 ) : ?>
              <a href="beli-langsung-non-fisik?customer=<?= $_GET['customer']; ?>" class="btn btn-default">Cash</a>
              <a href="beli-langsung-non-fisik?customer=<?= $_GET['customer']; ?>&r=MQ==" class="btn btn-primary">Piutang</a>
              <?php else : ?>
              <a href="beli-langsung-non-fisik?customer=<?= $_GET['customer']; ?>" class="btn btn-primary">Cash</a>
              <a href="beli-langsung-non-fisik?customer=<?= $_GET['customer']; ?>&r=MQ==" class="btn btn-default">Piutang</a>
              <?php endif; ?>
    ...
    <?php  
      ...
      $keranjangNonFisik = query("SELECT * FROM keranjang_non_fisik WHERE knf_id_kasir = $userId && knf_cabang = $sessionCabang ORDER BY knf_id ASC");
              ...
              <div class="row">
                ...
                <div class="col-md-4 col-lg-4">
                  <?php  
                    ...
                    $countTarikTunai = mysqli_query($conn, "SELECT * FROM keranjang_non_fisik WHERE knf_id_kasir = $userId && knf_cabang = $sessionCabang && knf_barang_nama LIKE '%tarik tunai%' ");
              ...
              <div class="table-auto">
                ...
                <table id="" class="table table-bordered table-striped">
                  ...
                  <?php 
                    ...
                    $sub_totalJasa              = $row['knf_harga_jual'] * $row['knf_qty'];
                    $sub_totalBeliNonFisik      = $row['knf_harga_beli'] * $row['knf_qty'];
          
                    if ( $row['knf_id_kasir'] === $_SESSION['user_id'] ) {
                  ...
                  <tr>
                      ...
                      <td>Rp. <?= number_format($row['knf_harga_beli'], 0, ',', '.'); ?></td>
                      <td>Rp. <?= number_format($row['knf_harga_jual'], 0, ',', '.'); ?></td>
                      <td style="text-align: center;"><?= $row['knf_qty']; ?></td>
                      <td>Rp. <?= number_format($sub_totalJasa, 0, ',', '.'); ?></td>
                      <td class="orderan-online-button">
                          <a href="#!" title="Edit Data">
                            <button class="btn btn-primary" name="" class="keranjang-pembelian" id="keranjang-harga-non-fisik" data-id="<?= $row['knf_id']; ?>">
                                <i class="fa fa-pencil"></i>
                            </button> 
                          </a>
                          <a href="beli-langsung-non-fisik-delete.php?id=<?= $row['knf_id']; ?>&customer=<?= $_GET['customer']; ?>&r=<?= $r; ?>" title="Delete Data" onclick="return confirm('Yakin dihapus ?')">
                    ...
                    <div class="col-md-6 col-lg-5">
                        ...
                        <!-- End Ongkir Statis untuk Inputan -->
                          ...
                          <tr>
                              ...
                              <td>
                                ...
                                <?php if ( $stkj['knf_id_kasir'] === $userId ) { ?>
                                  <input type="hidden" name="knf_nama[]" value="<?= $stkj['knf_nama']; ?>">
                                  <input type="hidden" name="knf_barang_kode[]" value="<?= $stkj['knf_barang_kode']; ?>">
                                  <input type="hidden" name="knf_barang_id[]" value="<?= $stkj['knf_barang_id']; ?>">
                                  <?php  
                                    $bnf_id = $stkj['knf_barang_id'];
                                  ...
                                  <?php  
                                    ...
                                    $queryTotal = "SELECT SUM(knf_harga_beli * knf_qty) AS total_harga_beli FROM keranjang_non_fisik WHERE knf_provider = $knf_provider";
                                  ...
                                  <input type="hidden" name="knf_harga_beli[]" value="<?= $stkj['knf_harga_beli']; ?>">
                                  <input type="hidden" name="knf_harga_jual[]" value="<?= $stkj['knf_harga_jual']; ?>">
                                  <input type="hidden" name="knf_qty[]" value="<?= $stkj['knf_qty']; ?>"> 
                                 <input type="hidden" name="knf_catatan[]" value="<?= $stkj['knf_catatan']; ?>"> 
                                  <input type="hidden" name="knf_id_kasir[]" value="<?= $stkj['knf_id_kasir']; ?>">
                                  <input type="hidden" name="knf_id_cek[]" value="<?= $stkj['knf_id_cek']; ?>">
                                <?php } ?>
                                <?php endforeach; ?> 

                                <input type="hidden" name="penjualan_invoice2" value="<?= $di; ?>">
                                <input type="hidden" name="invoice_customer_category" value="0">
                                ...
                                <input type="hidden" name="invoice_total_beli" value="0">
                                ...
                                <?php  
                                  $countNonFisik = mysqli_query($conn, "SELECT * FROM keranjang_non_fisik WHERE knf_id_kasir = $userId && knf_cabang = $sessionCabang");
                      ...
                      <div class="payment">
                        <?php  
                            ...
                            $countProvider = "SELECT SUM(knf_provider) AS total_provider FROM keranjang_non_fisik WHERE knf_id_kasir = $idKasirKeranjang && knf_cabang = $sessionCabang";
  ...
  <script>
    $(document).ready(function(){
        ...
        $('#example2 tbody').on( 'click', '.tblInsert', function () {
            ...
            window.location.href = "beli-langsung-add-non-fisik.php?id="+ data0 + "&customer=<?= $_GET['customer']; ?>&r=<?= $r; ?>";
        });
...
<script>
  $(document).ready(function(){
      ...
      $(document).on('click','#keranjang-harga-non-fisik',function(e){
          ...
          $.post('beli-langsung-edit-harga-non-fisik.php?customer=<?= $tipeHarga; ?>',
            {id:$(this).attr('data-id')},
            function(html){
