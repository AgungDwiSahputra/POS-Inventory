<?php 
  ...
  $keranjang_transfer_cabang = $keranjang['keranjang_transfer_cabang'];
?>
	
	<input type="hidden" name="keranjang_id" value="<?= $id; ?>">
  <input type="hidden" name="barang_id" value="<?= $barang_id; ?>">
  <input type="hidden" name="keranjang_transfer_cabang" value="<?= $sessionCabang; ?>">
	<div class="form-group">
        ...
        <select class="form-control select2bs4 pilihan-marketplace" required="" name="barang_sn_id">
          <option selected="selected" value="">-- Pilih --</option>
            <?php  
              $noSn = query("SELECT * FROM barang_sn WHERE barang_sn_status = 1 && barang_kode_slug = '".$barang_kode_slug."' && barang_sn_cabang = $keranjang_transfer_cabang ");
            ?>
